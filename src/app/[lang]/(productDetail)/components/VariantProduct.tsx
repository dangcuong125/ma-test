import { Stack, Box, styled , Typography, Link, Grid, alpha, IconButton, useTheme, Divider, Button, Card, Tab } from "@mui/material";
        ...
        <Grid container height={'100%'}>
            ...
            <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
                <Stack spacing={4} ml="50px">
                    <Typography sx={{ width: "81px", fontSize: "12px", borderRadius: "6px", textAlign: "center", fontWeight: 400, color: "#B72136", backgroundColor: "#ff484229" }}>Khuyến mãi</Typography>
                    <Stack>
                        <Typography variant="h3">Súp lơ xanh</Typography>
                        <Typography variant="h4" sx={{ color: "#98A1B3" }}>Súp lơ xanh trồng tại Đà Lạt</Typography>
                    </Stack>
                    <Stack>
                        <Typography variant="h3">50.000VND</Typography>
                        <Typography variant="h4" sx={{ color: "#98A1B3", textDecoration: "line-through" }} >50.000VND</Typography>
                    </Stack>
                    <Divider />
                    <Stack display={"flex"} direction={"row"} justifyContent={"space-between"}>
                        <Typography variant="subtitle1" sx={{ color: "#666E80" }}>Chọn màu sắc</Typography>
                        <Controller
                            name="color"
                            control={control}
                            render={({ field  }) => (
                    ...
                    <Stack display={"flex"} direction={"row"} justifyContent={"space-between"}>
                        <Typography variant="subtitle1" sx={{ color: "#666E80" }}>Chọn khối lượng</Typography>
                        <Stack width={"50%"} >
                            ...
                            <Typography sx={{textDecoration:"underline", textAlign:"end",py:"6px"}}>Bảng khối lượng</Typography>
                        </Stack>
                    </Stack>
                    ...
                    <Stack display={"flex"} direction={"row"} justifyContent={"space-between"}>
                        <Incrementer
                            ...
                            available={72}
                            onIncrementQuantity={() => setValue('quantity', watch().quantity + 1)}
                            onDecrementQuantity={() => setValue('quantity', watch().quantity - 1)}
                        />
                        <Button endIcon={ <Iconify icon={'formkit:arrowright'} width={24} height={24} />} sx={{ background: "#1F8A70", color: "white", width: "60%", borderRadius: "30px", fontSize: "18px" }}>
                            Thêm vào giỏ hàng
                        </Button>
                    </Stack >
                    <Stack border={"1px solid #E4E4E4"} spacing={1} p={3} borderRadius={1}>
                        <Stack display={"flex"} direction={"row"} spacing={2}>
                            <Image sx={{ pt: "9px", minWidth: "24px", height: "27px" }} src={'/Frame.svg'} />
                            <Stack>
                                <Typography fontSize={24} fontWeight={600}>Giao hàng miễn phí</Typography>
                                <Typography color={"#726C6C"} fontSize={18}>Enter your Postal code for Delivery Availability</Typography>
                            </Stack>
                        </Stack>
                        <Divider />
                        <Stack display={"flex"} direction={"row"} spacing={2}>
...
const leftIcon = (isRTL?: boolean) => (
  ...
  background:"#F3F3F3",
      ...
      >
        ...
        <Typography variant="body2" component="span" sx={{ fontWeight: 700, fontSize: "24px", width: 40, textAlign: 'center' }}>
          {quantity}
        </Typography>
